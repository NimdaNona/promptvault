...
export const importSessions = pgTable("import_sessions", {
	...
	processedPrompts: integer("processed_prompts").default(0).notNull(),
	failedPrompts: integer("failed_prompts").default(0).notNull(),
	error: text(),
	metadata: jsonb(),
	startedAt: timestamp("started_at", { mode: 'string' }).defaultNow().notNull(),
	completedAt: timestamp("completed_at", { mode: 'string' }),
}, (table) => [
	index("import_sessions_platform_idx").using("btree", table.platform.asc().nullsLast().op("text_ops")),
	index("import_sessions_status_idx").using("btree", table.status.asc().nullsLast().op("text_ops")),
	index("import_sessions_started_at_idx").using("btree", table.startedAt.asc().nullsLast().op("timestamp_ops")),
